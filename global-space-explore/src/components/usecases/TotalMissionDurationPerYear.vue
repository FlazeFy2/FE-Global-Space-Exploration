            ...
            data_raw['Year'].forEach((year, index) => {
                ...
                if (!isNaN(days)) {
                    if (!year_mission_days[year]) {
                        year_mission_days[year] = 0
                    }

                    year_mission_days[year] += days
                }
            });

            const grouped_data = Object.entries(year_mission_days).map(([year,total]) => {
                return { context: year, total: total }
            });

            grouped_data.sort((a, b) => parseInt(a.context) - parseInt(b.context))

            labels.value = grouped_data.map(d => d.context);
            series.value = [{ name: "Duration (in Days)", data: grouped_data.map(d => d.total) }]
        } catch (error) {
